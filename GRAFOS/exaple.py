...
class Grafo:
    ...
    def ruta_corta_dijkstra(self, source):
        shortest_paths = nx.single_source_dijkstra_path(self.grafo, source=source)
        for target, path in shortest_paths.items():
            if target != source:
                print(f'Ruta más corta desde {source} a {target}: {path}')
...
grafo.ruta_corta_dijkstra('Rioacha')
